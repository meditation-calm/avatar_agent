...
from src.chat_engine.data_models.runtime_data.data_store import DataStore, DataStoreType
from src.chat_engine.data_models.runtime_data.event_model import EventData
from src.chat_engine.data_models.runtime_data.time_unit_type import TimeUnitType
...
class DataBundle:
    """
    实际的数据包容器，包含具体的数据和元数据：
        _definition：关联的数据包定义
        metadata：元数据字典
        events：事件数据列表
        _data_entries：数据条目列表
        data：实际数据存储列表
        start_of_stream：流开始标记
        end_of_stream：流结束标记
    """
