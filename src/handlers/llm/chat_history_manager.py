...
from src.engine_utils.media_utils import ImageUtils
...
class ChatHistory:
    ...
    def generate_next_messages(self, chat_text, images):
        def history_to_message(message: HistoryMessage):
            return {
                "role": name_dict[message.role],
                "content": filter_text(message.content),
            }
        history = self.message_history
